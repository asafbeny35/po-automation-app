...
/// טופס יצירה/עריכה גנרי — אותו EditFormKind המשותף, אותם payloads לשרת.
struct ClassicRecordFormView: View {
        ...
        NavigationStack {
                ...
                if let validationError {
                    Section {
                        Label(validationError, systemImage: "exclamationmark.triangle.fill")
                            ...
                            .foregroundColor(.red)
                            .accessibilityIdentifier("classic-form-error")
                    }
                }
                Section {
                    ForEach(kind.fields) { field in
                        if let options = field.options {
                            HStack {
                                ...
                                .accessibilityIdentifier("classic-form-\(field.key)")
                            }
                        } else if field.multiline {
                            VStack(alignment: .leading, spacing: 6) {
                                Text(field.label).font(.caption.weight(.medium)).foregroundColor(.secondary)
                            ...
                            HStack {
                                Text(field.label).foregroundColor(.secondary)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(isSaving)
                    .accessibilityIdentifier("classic-form-save")
                }
            }
        }
        .onAppear { populate() }
    ...
    private func populate() {
        ...
        if let existing {
            for field in kind.fields { values[field.key] = existing[field.key] }
        }
    }
    ...
    private func save() async {
        ...
        case .customer:
            ...
            succeeded = await session.perform(
                ...
                try await api.postJSON(existingRecord == nil ? "customers-create" : "customers-update",
