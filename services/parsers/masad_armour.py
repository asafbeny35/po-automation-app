...
CUSTOMER_NAME = 'מצדה ארמור שיא בע"מ'
...
def _amount(value: str) -> float:
    token = normalize_ws(value or "").replace(",", "").replace("₪", "").replace('ש"ח', "").replace("שח", "")
    try:
        return float(token)
    except Exception:
        return 0.0
...
def _normalize_phone(value: str) -> str:
    ...
    if not digits:
        return ""
    if len(digits) == 9 and digits.startswith(("4", "7", "5")):
        digits = "0" + digits
    if len(digits) == 10:
...
def parse(text: str):
    if "רומרא הדצמ" not in text and "מצדה ארמור" not in text:
        return None
    ...
    delivery_block = re.search(
        r"בן יעקב פתרונות טקסטיל\s*\n([^\n]+)\n([^\n]+)\n([^\n]+)\n([^\n]+)\s*:\s*ןופלט",
        flat,
    )
    if delivery_block:
        for idx in range(1, 5):
            part = delivery_block.group(idx).strip(" ,")
            if part:
                address_parts.append(_reverse_words(part))
    header["delivery_address"] = normalize_ws(", ".join(address_parts))

    item = POItem(description="פריט לא זוהה", quantity=1, unit_price=0, line_total=0, sku="")
    item_match = re.search(
        r"([0-9,]+\.\d{2})\s+ח\"ש\s+([0-9,]+\.\d{2})\s+מ\s+([0-9,]+\.\d{2})\s+מ\s+([0-9,]+\.\d{2})\s+(\d{2}/\d{2}/\d{2})\s+(.+?)\s+\*(\d+)\*\s+(\d+)\s+1",
        flat,
        re.DOTALL,
    )
    if item_match:
        item = POItem(
            sku=item_match.group(8),
            description=_reverse_words(item_match.group(6)),
